...
class RAGAgent:
    ...
    def ask(self, session_id: str, query: str) -> str:
        ...
        prompt = (
            f"Ты эксперт по GPSS Studio. Ответ должен быть четким, понятным, "
            f"конструктивным, верным. Если пользователь просит, можно "
            f"привести пример кода и источник, откуда взял данные. От"
            f"Используй контекст для ответа на вопрос.\n\n"
            f"Контекст:\n{context}\n\n"
            f"История чата:\n{history_text}\n\n"
        ...
        prompt = f"""
Ты — эксперт по имитационному моделированию в среде GPSS Studio. Твоя задача — давать точные, конструктивные и проверенные ответы на вопросы пользователей, опираясь исключительно на предоставленный контекст (фрагменты официального руководства пользователя).  
